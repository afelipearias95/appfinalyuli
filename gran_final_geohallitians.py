...
df_bopd.rename(columns={'Day': 'Date'}, inplace=True)
...
def create_bar_chart(df_minus_gas):
    ...
    fig = go.Figure(data=[go.Bar(
        ...
        y=df_minus_gas['UWI'],
        orientation='h',
        width=0.4  # Ajusta el ancho de las barras para reducir la altura
    )])
    
    # Establece el color de las barras
    fig.update_traces(
        marker=dict(color='rgba(0, 204, 255, 0.2)')
    )
        ...
        dcc.Graph(
            ...
            figure={
                ...
                'layout': go.Layout(
                    ...
                    xaxis=dict(tickangle=45, tickfont=dict(size=9, color='white'), title_font=dict(color='white')),
                    yaxis=dict(tickangle=45, tickfont=dict(size=8, color='white'), title_font=dict(color='white'))
                ),
            },
...
header_cell_style = {
    ...
    'text-align': 'left',
    'color': 'black',
    'font-size': '12px',  # Tamaño de fuente deseado para las celdas de encabezado
}
...
def tabla_critical():
    ...
    filtered_df_subset = filtered_df[['UWI', 'BOPD', 'Average_BOPD', 'Date']]
    ...
    filtered_df_subset = filtered_df_subset.sort_values(by='Date', ascending=False)
...
# Define la imagen que deseas agregar
image_src = "assets\campana2.png"

# Agrega el título y la imagen en un div con CSS
title_with_image = html.Div([
    html.Img(src=image_src, style={'width': '40px', 'vertical-align': 'middle', 'margin-right': '10px'}),
    html.Div("Critical Alerts", style={
        'display': 'inline',
        'fontWeight': 'bold',
        'color': 'white',
...
def count_consecutive_days():
    ...
    query = """
    SELECT
        wmu.UWI,  -- Agregar la columna UWI de wells_master_updated
        cvu.Well_Id,
        DATE(cvu.Date) AS Start_Date,
        cvu.Presion_Intake,
        cvu.Temperatura_Intake,
        cvu.Pressure_Discharge,
        cvu.Temperatura_Motor
    FROM
        critical_variables_updated cvu
    INNER JOIN
        wells_master_updated wmu ON cvu.Well_Id = wmu.Well_Id  -- Realizar una unión con wells_master_updated
    WHERE
        cvu.Run_Status = 0 AND TIME(cvu.Date) = '00:00:00'
    ORDER BY
        cvu.Well_Id, Start_Date ASC;
    """
    ...
    # Calcular la diferencia en días entre filas consecutivas
    df['Days_Since_Last'] = (df['Start_Date'] - df['Start_Date'].shift(1)).dt.days

    # Inicializar variables
    current_well = None
    current_date = None
    start_date = None
    end_date = None  # Agregar una variable para el End_Date
    consecutive_days = 0
    results = []

     # Recorrer el DataFrame y contar los días consecutivos por pozo y fecha
    for index, row in df.iterrows():
        if current_well is None:
            current_well = row['UWI']
            start_date = row['Start_Date']
            current_date = row['Start_Date']
            end_date = row['Start_Date']  # Inicializar End_Date
            consecutive_days = 1

        elif row['UWI'] == current_well and (row['Start_Date'] - current_date) <= pd.Timedelta(days=1):

            consecutive_days += 1
            end_date = row['Start_Date']  # Actualizar End_Date
            current_date = row['Start_Date']
        else:
            end_date = start_date + pd.Timedelta(days=consecutive_days-1)
            results.append((current_well, start_date, end_date, consecutive_days-1))
            current_well = row['UWI']
            start_date = row['Start_Date']
            end_date = row['Start_Date']  # Reiniciar End_Date
            current_date = row['Start_Date']
            consecutive_days = 1

    # Crear un DataFrame con los resultados
    result_df = pd.DataFrame(
        results, columns=['UWI', 'Start_Date', 'End_Date', 'Consecutive_Days']
    )

    # Eliminar filas donde Consecutive_Days sea igual a 0
    result_df = result_df[result_df['Consecutive_Days'] > 0]

    return result_df

# Llamar a la función para contar los días consecutivos
consecutive_days_df = count_consecutive_days()

consecutive_days_df['Start_Date'] = consecutive_days_df['Start_Date'].dt.date
consecutive_days_df['End_Date'] = consecutive_days_df['End_Date'].dt.date

consecutive_days_df.rename(columns={'Consecutive_Days': 'Inactive_Days'}, inplace=True)


def tabla_2_critical():

    # Ordena el DataFrame por la columna "Start Date" de mayor a menor
    sorted_df = consecutive_days_df.sort_values(by='Start_Date', ascending=False)

    # Limitar el DataFrame a los primeros 50 valores
    sorted_df = sorted_df.head(10)

    return html.Table(
        children=[
            # Encabezados de la tabla
            html.Tr([html.Th(col, style=header_cell_style) for col in sorted_df.columns], style=table_style),
        ] +
        # Filas de datos
        [
            html.Tr([html.Td(sorted_df.iloc[i][col], style=cell_style) for col in sorted_df.columns], style=table_style)
...
#         wmu.UWI,  -- Agregar la columna UWI de wells_master_updated
#         cvu.Date,
#         cvu.Presion_Intake AS P_Intake,
#         cvu.Temperatura_Intake AS T_Intake,
#         cvu.Pressure_Discharge AS P_Discharge,
#         cvu.Temperatura_Motor AS T_Motor,
#         cvu.Run_Status
#     FROM
#         critical_variables_updated cvu
#     INNER JOIN
...
#     # Elegir las columnas relevantes para la detección de anomalías
#     columns_for_anomaly_detection = ['P_Intake', 'T_Intake', 'T_Motor', 'P_Discharge']

#     # Crear un modelo Isolation Forest
#     model = IsolationForest(contamination=0.05)  # Puedes ajustar el valor de 'contamination'

#     # Entrenar el modelo en los datos
#     model.fit(df[columns_for_anomaly_detection])

#     # Predicción de anomalías
#     anomalies = model.predict(df[columns_for_anomaly_detection])

#     # Filtrar por valores de 'Pressure_Discharge' mayores a 3000
#     anomalies_df = df[(anomalies == -1) & (df['P_Discharge'] > 3000)]

#     # Ordenar el DataFrame por fecha de manera descendente
...
#     children=[
#         # Encabezados de la tabla
#         html.Tr([html.Th(col, style=header_cell_style) for col in anomalies_df.columns], style=table_style),
#     ] +
#     # Filas de datos
#     [
#         html.Tr([html.Td(anomalies_df.iloc[i][col], style=cell_style) for col in anomalies_df.columns], style=table_style)
...
def tabla_3_critical():
    ...
    # Limitar el DataFrame a las primeras 10 filas y las columnas seleccionadas
    display_df = df[columns_to_display].head(10)

    return html.Table(
        children=[
            # Encabezados de la tabla
            html.Tr([html.Th(col, style=header_cell_style) for col in display_df.columns], style=table_style),
        ] +
        # Filas de datos
        [
            html.Tr([html.Td(display_df.iloc[i][col], style=cell_style) for col in display_df.columns], style=table_style)
...
alertas_card = html.Div(
    ...
    children=[
        title_with_image,  # Aquí utilizamos el título con la imagen
        tabla_critical(),
        html.Div(style={'height': '10px'}),  # Espacio de 20px entre las tablas
        tabla_2_critical(),  # Agrega la segunda tabla basada en consecutive_days_df
        html.Div(style={'height': '10px'}),  # Espacio de 20px entre las tablas
        tabla_3_critical() 
    ],
)
...
def create_map(selected_sistemas):
    ...
    # En la función create_map, agrega el ID al componente dl.Map
    map_ = dl.Map(
        id='map',  # Agrega un ID al componente del mapa
...
wells_location_card = html.Div(
    ...
    children=[
        ...
        html.Div(
            style={'justify-content': 'flex-end','align-items': 'right'},
            children=[
                # Dropdown para seleccionar el sistema de levantamiento
                dcc.Dropdown(
                    id='sistema-filtro',
                    options=[
                        {'label': 'BES', 'value': 'BES'},
                        {'label': 'BME', 'value': 'BME'},
                        {'label': 'GAS LIFT', 'value': 'GAS LIFT'},
                        {'label': 'PCP', 'value': 'PCP'},
                        {'label': 'All ALS', 'value': 'All ALS'}
                    ],
                    style={'backgroundColor': '#000000'},
                    value='All ALS',  # Valor predeterminado
                    multi=True  # Permite selección múltiple
                )
            ]
        ),
        # Luego, en la función create_map() en el layout, utiliza este ID
        html.Div(
            id='map-container',
            children= create_map(['All ALS']),  # Cambia aquí para proporcionar un valor predeterminado
        )
        ]
)


# Agregar la política de seguridad de contenido (CSP) para permitir iframes
...
def brent_price():
    return html.Iframe(
        src="https://es.investing.com/commodities/brent-oil",
        width="450px",
        height="1250px",
        style={'transform': 'scale(0.65)'},
        sandbox="allow-same-origin",  # Permitir contenido del mismo origen
    )

# Agrega el título de la tarjeta aquí
Brent_realtime_card = html.Div(
    style={
        'height': '300px',
        'textAlign': 'center',
        'border-radius': '10px',
    ...
    children=[
        ...
        html.Div(  # Contenedor para el iframe con margen superior
            brent_price(),
            style={'marginTop': '-210px', 'marginLeft': '-80px', 'marginRight': '-80px', 'marginBottom': '-20px'}  # Ajusta los márgenes según sea necesario
        ),
    ],
)
...
def wti_price():
    return html.Iframe(
        src="https://es.investing.com/commodities/crude-oil",
        width="450px",
        height="1200px",
        style={
...
Wti_realtime_card = html.Div(
    ...
    children=[
        ...
        html.Div(  # Contenedor para el iframe con margen superior
            wti_price(),
            style={'marginTop': '-200px', 'marginLeft': '-80px', 'marginRight': '-80px', 'marginBottom': '-20px'}  # Ajusta los márgenes según sea necesario
        ),
    ],
)
...
#         html.A("Download Handbook", id="download-link", href="assets/Handbook_Geohallitians_2023.pdf", download="Handbook_Geohallitians_2023.pdf", style={'color': '#007BFF','position': 'absolute', 'top': '10px', 'right': '10px'}, className='handbook-button')
#     ]),
...
app.layout = html.Div(style={'fontFamily': 'Lato', 'display': 'flex', 'flexDirection': 'column'}, children=[
    ...
    html.Div(style={'position': 'relative', 'className': 'button-container'}, children=[
        ...
        html.Button('Export PPT', id='btn-pptx', n_clicks=0, className='export-button'),
        html.Button('Export PNG', id='btn-png', n_clicks=0, className='export-button'),
        html.A("Download Handbook", id="download-link", href="assets/Handbook_Geohallitians_2023.pdf", download="Handbook_Geohallitians_2023.pdf", className='handbook-button')
    ]),
    ...
    html.Div(style={'display': 'flex', 'backgroundColor' : '#131313', 'border-radius':'7px'}, children=[
        ...
        html.Div(f'Last Updated: {formatted_time} (Colombia Time)', style={'align': 'center', 'color': '#007BFF', 'margin': '1px 0', 'align-items': 'top', 'fontSize': '14px', 'fontWeight': 'bold'}),
    ]),
    ...
    html.Div(style={'display': 'flex'}, children=[
        ...
        html.Div(style={'width': '17%', 'margin-top': '0px'}, children=[
                html.Div(style={'backgroundColor': '#131313', 'border-radius': '7px', 'max-height': '50vh'}, children=[
                    html.H3("Date Filters", style={'textAlign': 'center', 'color': 'white', 'borderBottom': '1px solid white', 'paddingTop': '10px', 'paddingBottom': '10px'}),
                    ...
                    dcc.Dropdown(
                        ...
                        value='Historical',  # Valor por defecto
...
def download_handbook(n_clicks):
    if n_clicks is not None:
        return "assets/Handbook_Geohallitians_2023.pdf"  # Reemplaza con la ruta correcta en tu servidor
...
# Define la función del callback para actualizar el mapa
@app.callback(
    Output('map', 'children'),
    [Input('sistema-filtro', 'value')]
)
def update_map(selected_sistemas):
    if 'All ALS' in selected_sistemas:
        filtered_df = df_sumas_por_pozo
    else:
        filtered_df = df_sumas_por_pozo[df_sumas_por_pozo['Sistema_Levantamiento'].isin(selected_sistemas)].copy()
